...
namespace MyLaboratorio.Data
{
	public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if (res == DefaultDataControllerStream)
                	res = null;
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("MyLaboratorio.Controllers.{0}.xml", controller));
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("MyLaboratorio.{0}.xml", controller));
                if (res == null)
                {
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
            	t = typeof(MyLaboratorio.Data.Controller);
            Stream res = t.Assembly.GetManifestResourceStream(String.Format("MyLaboratorio.Reports.{0}", name));
            if (res == null)
            	res = t.Assembly.GetManifestResourceStream(String.Format("MyLaboratorio.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                ControllerConfiguration config = MyLaboratorio.Data.Controller.CreateConfigurationInstance(t, controller);
